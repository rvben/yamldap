...
fn create_test_directory(num_users: usize) -> Directory {
    let mut entries = vec![
        ...
        YamlEntry {
            ...
            attributes: {
                let mut attrs = HashMap::new();
                attrs.insert("ou".to_string(), serde_yaml::Value::String("users".to_string()));
                attrs
            },
        },
    ];
    ...
    for i in 0..num_users {
        entries.push(YamlEntry {
            ...
            object_class: vec!["top".to_string(), "person".to_string(), "inetOrgPerson".to_string()],
            attributes: {
                let mut attrs = HashMap::new();
                attrs.insert("uid".to_string(), serde_yaml::Value::String(format!("user{}", i)));
                attrs.insert("cn".to_string(), serde_yaml::Value::String(format!("User {}", i)));
                attrs.insert("sn".to_string(), serde_yaml::Value::String(format!("Surname{}", i)));
                attrs.insert("mail".to_string(), serde_yaml::Value::String(format!("user{}@example.com", i)));
                attrs.insert("userPassword".to_string(), serde_yaml::Value::String("password123".to_string()));
                attrs
            },
        });
    }
...
fn benchmark_entry_lookup(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, _| {
            b.iter(|| {
                dir.get_entry(black_box("uid=user500,ou=users,dc=example,dc=com"))
            });
        });
    }
...
criterion_group!(benches, benchmark_entry_lookup, benchmark_search_all, benchmark_filter_search, benchmark_auth_lookup);
