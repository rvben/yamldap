...
mod tests {
    ...
    fn test_attribute_value_as_string() {
        assert_eq!(AttributeValue::String("test".to_string()).as_string(), "test");
        ...
        assert_eq!(AttributeValue::Dn("cn=test,dc=example,dc=com".to_string()).as_string(), "cn=test,dc=example,dc=com");
        
        let binary_data = vec![1, 2, 3, 4];
        let encoded = AttributeValue::Binary(binary_data.clone()).as_string();
        assert_eq!(encoded, base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &binary_data));
        
        let dt = chrono::Utc.with_ymd_and_hms(2023, 12, 25, 10, 30, 0).unwrap();
        assert_eq!(AttributeValue::GeneralizedTime(dt).as_string(), "20231225103000Z");
    }

    #[test]
    fn test_attribute_value_as_bytes() {
        assert_eq!(AttributeValue::String("test".to_string()).as_bytes(), b"test".to_vec());
        ...
        assert_eq!(AttributeValue::Dn("cn=test".to_string()).as_bytes(), b"cn=test".to_vec());
        
        let binary_data = vec![1, 2, 3, 4];
        assert_eq!(AttributeValue::Binary(binary_data.clone()).as_bytes(), binary_data);
        
        let dt = chrono::Utc.with_ymd_and_hms(2023, 12, 25, 10, 30, 0).unwrap();
        assert_eq!(AttributeValue::GeneralizedTime(dt).as_bytes(), b"20231225103000Z".to_vec());
    }
    ...
    fn test_ldap_entry_from_yaml_entry_simple() {
        let yaml_entry = crate::yaml::YamlEntry {
            ...
            attributes: [
                ("cn".to_string(), serde_yaml::Value::String("test".to_string())),
                ("sn".to_string(), serde_yaml::Value::String("User".to_string())),
                ("uid".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(12345))),
    ...
    fn test_ldap_entry_from_yaml_entry_with_sequences() {
        let yaml_entry = crate::yaml::YamlEntry {
            ...
            attributes: [
                ("cn".to_string(), serde_yaml::Value::String("test".to_string())),
                ("member".to_string(), serde_yaml::Value::Sequence(vec![
    ...
    fn test_ldap_entry_from_yaml_entry_with_float() {
        let yaml_entry = crate::yaml::YamlEntry {
            ...
            attributes: [
                ("score".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(3.14))),
            ].into_iter().collect(),
        };
    ...
    fn test_guess_attribute_syntax() {
        ...
        assert!(matches!(
            guess_attribute_syntax("createTimestamp", &AttributeValue::String("test".to_string())),
            AttributeSyntax::GeneralizedTime
        ));
        assert!(matches!(
            guess_attribute_syntax("modifyTimestamp", &AttributeValue::String("test".to_string())),
            AttributeSyntax::GeneralizedTime
        ));
    ...
    fn test_attribute_value_equality() {
        ...
        assert_ne!(AttributeValue::Boolean(true), AttributeValue::Boolean(false));
    ...
    fn test_attribute_syntax_equality() {
        ...
        assert_eq!(AttributeSyntax::GeneralizedTime, AttributeSyntax::GeneralizedTime);
    }
}
