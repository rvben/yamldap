    ...
    pub fn matches(&self, entry: &LdapEntry) -> bool {
        match self {
            ...
            LdapFilter::Or(filters) => {
                filters.iter().any(|f| f.matches(entry))
            }
        ...
        if filter_str.starts_with('(') && filter_str.ends_with(')') {
            ...
            if value.contains('*') && value != "*" {
                ...
                let substring = SubstringFilter {
                    initial: if parts[0].is_empty() { None } else { Some(parts[0].to_string()) },
                    any: parts[1..parts.len() - 1].iter().map(|s| s.to_string()).collect(),
                    final_: if parts[parts.len() - 1].is_empty() { None } else { Some(parts[parts.len() - 1].to_string()) },
                };
...
mod tests {
    ...
    fn test_parse_filter() {
        ...
        assert_eq!(filter, LdapFilter::Equality("uid".to_string(), "john".to_string()));
    }
}